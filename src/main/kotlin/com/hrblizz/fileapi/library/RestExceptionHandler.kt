...
import com.hrblizz.fileapi.library.property.AppProperties
...
class RestExceptionHandler(
    ...
    fun handleFileAlreadyExists(ex: FileAlreadyExistsException): ResponseEntity<Any> {
        val errorStatus = HttpStatus.CONFLICT
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null,
            listOf(ErrorMessage(ex.message + " already exists")),
            errorStatus.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), errorStatus)
    }
    ...
    fun handleSizeLimitExceededE(ex: MaxUploadSizeExceededException): ResponseEntity<Any> {
        val errorStatus = HttpStatus.BAD_REQUEST
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null,
            listOf(ErrorMessage("Max file size limit is:" + appProperties.maxFileSize + "and max file request size limit is " + appProperties.maxRequestSize)),
            errorStatus.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), errorStatus)
    }
